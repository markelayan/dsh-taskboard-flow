            ...
            disposeSessionMsgTool = toolsSvcMsg.register({
              ...
              async execute(args, exec) {
                try {
                  ...
                  if (!target) return { text: JSON.stringify({ ok: false, error: 'target session id required (use action "list" to discover live sessions)' }) }
                  if (!message) return { text: JSON.stringify({ ok: false, error: 'message text required' }) }
                  if (target === sid) return { text: JSON.stringify({ ok: false, error: 'self-send refused — target must be another session' }) }
                  let agent = agentCtx.agents.get(target)
                  let resumed = false
                  if (!agent && args?.resumeIfDead === true) {
                    try {
                      const handle = await agentCtx.agents.resume({ sessionId: target })
                      agent = handle?.agent ?? handle
                      resumed = true
                    } catch (e) {
                      return { text: JSON.stringify({ ok: false, error: `resume ${target} failed: ${e?.message}` }) }
                    }
                  }
                  if (!agent || typeof agent.inject !== 'function') {
                    return { text: JSON.stringify({ ok: false, error: `session ${target} not live${args?.resumeIfDead === true ? '' : ' (retry with resumeIfDead: true to resurrect it)'}` }) }
                  }
                  const text = `[session-message] From session ${sid}:\n\n${message}`
                  pushContextNote(target, `sm-${randomUUID()}`, text)
                  installContextNotes(agent)
                  // v0.6.1: the wake carries the FULL payload — steer renders
                  // it visibly in the target conversation; a pointer-only
                  // nudge told humans "full text in your runtime context"
                  // while showing them nothing. Returns honest nudgeVia so
                  // the caller result never overstates delivery. Status is
                  // read ONCE (race observed live: target flipped idle→running
                  // mid-send, which double-reading turned into a contradictory
                  // nudgeVia=steer + delivery=context-only result).
                  const idleAtSend = agent.status === 'idle'
                  let nudgeVia = 'none'
                  if (args?.wake !== false && idleAtSend) {
                    const wake = {
                  ...
                  // v0.6.2 (per user, revised from a deferred-timer draft):
                  // busy target + wake requested → inject the FULL text as a
                  // plugin-source notice — the SAME delivery path as the
                  // context/compression nudge (tguard). It renders VISIBLY
                  // in the target conversation immediately, is mid-turn safe,
                  // and starts no turn. Idle targets already got the wake.
                  let noticeInjected = false
                  if (!idleAtSend && args?.wake !== false && typeof agent.inject === 'function') {
                    try {
                      agent.inject({
                        id: `msg-smn-${randomUUID()}`,
                        role: 'user',
                  ...
                  log(`[taskboard-flow] session_message ${sid} → ${target} (${delivery})${resumed ? ' [resumed]' : ''}`)
                  return { text: JSON.stringify({ ok: true, from: sid, to: target, targetStatus: agent.status ?? 'unknown', delivery, nudgeVia, noticeInjected, note, resumed }) }
                } catch (err) {
                  return { text: JSON.stringify({ ok: false, error: err?.message ?? String(err) }) }
                }
              },
